...
var Server = exports.Server = function (_EventEmitter) {
    ...
    function Server(options) {
        ...
        _this2.socksMode = !!options.socksMode;
        ...
        _this2.server.on('connection', _this2.onConnection.bind(_this2));
    ...
    _createClass(Server, [{
        ...
        value: function onConnection(socket) {
            ...
            socket.on('error', function (err) {
                ...
                if (_this3.listenerCount('error') === 1) {
                    _this3.log(handlerOpts.id, 'Source socket emitted error: ' + (err.stack || err));
                }
            });
        }
        ...
        value: function onRequest(request, response) {
            ...
            this.prepareRequestHandling(request).then(function (result) {
                ...
                if (handlerOpts.customResponseFunction) {
                    _this4.log(handlerOpts.id, 'Using HandlerCustomResponse');
                    handler = new _handler_custom_response2.default(handlerOpts);
                } else {
                    _this4.log(handlerOpts.id, 'Using HandlerForward');
        ...
        value: function onConnect(request, socket, head) {
            ...
            this.prepareRequestHandling(request).then(function (result) {
                ...
                if (handlerOpts.upstreamProxyUrlParsed) {
                    _this5.log(handlerOpts.id, 'Using HandlerTunnelChain');
                    handler = new _handler_tunnel_chain2.default(handlerOpts);
                } else {
                    _this5.log(handlerOpts.id, 'Using HandlerTunnelDirect');
                    handler = new _handler_tunnel_direct2.default(handlerOpts);
                }

                _this5.handlerRun(handler);
            }).catch(function (err) {
                _this5.failRequest(request, err, handlerOpts);
            });
        }
        ...
        value: function prepareRequestHandling(request) {
            ...
            return Promise.resolve().then(function () {
                ...
                if (request.method === 'CONNECT') {
                    ...
                    _this6.stats.connectRequestCount++;
                } else {
                    // The request should look like:
                ...
                return _this6.prepareRequestFunction(funcOpts);
                ...
                if (funcResult && funcResult.customResponseFunction) {
                    _this6.log(handlerOpts.id, 'Using custom response function');
                ...
                if (handlerOpts.upstreamProxyUrlParsed) {
                    _this6.log(handlerOpts.id, 'Using upstream proxy ' + (0, _tools.redactParsedUrl)(handlerOpts.upstreamProxyUrlParsed));
                }
        ...
        value: function handlerRun(handler) {
            ...
            handler.once('close', function (_ref) {
                ...
                _this7.emit('connectionClosed', {
                    connectionId: handler.id,
                    stats: stats
                });
                delete _this7.handlers[handler.id];
                _this7.log(handler.id, '!!! Closed and removed from server');
            });
            ...
            handler.once('tunnelConnectResponded', function (_ref2) {
                ...
                _this7.emit('tunnelConnectResponded', {
                    connectionId: handler.id,
                    response: response,
                    socket: socket,
                    head: head
        ...
        value: function listen(callback) {
            ...
            var promise = new Promise(function (resolve, reject) {
                ...
                var onError = function onError(err) {
                    _this8.log(null, 'Listen failed: ' + err);
                    removeListeners();
                    reject(err);
                };
                var onListening = function onListening() {
                    _this8.port = _this8.server.address().port;
                    _this8.log(null, 'Listening...');
                    removeListeners();
                    resolve();
                };
                var removeListeners = function removeListeners() {
                    _this8.server.removeListener('error', onError);
                    _this8.server.removeListener('listening', onListening);
                };

                _this8.server.on('error', onError);
                _this8.server.on('listening', onListening);
